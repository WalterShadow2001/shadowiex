<#
.SYNOPSIS
    SHADOWIEX v13.0 - Professional Edition with MAS Offline
.DESCRIPTION
    Herramienta profesional de instalacion y activacion.
    MAS incluido - funciona 100% OFFLINE.
    Distribuir junto con MAS_AIO.cmd
.NOTES
...
# ============================================
# MAS OFFLINE - Buscar archivo local
# ============================================
...
function Deploy-MAS {
    ...
    # Si no existe, preguntar si descargar
    $R = [System.Windows.Forms.MessageBox]::Show(
        "No se encontro MAS_AIO.cmd`n`nDeseas descargar MAS desde GitHub?`n(necesitas conexion a internet)",
    ...
    if ($R -eq [System.Windows.Forms.DialogResult]::Yes) {
        ...
        try {
            ...
            $destPath = Join-Path $env:TEMP "SHADOWIEX_MAS.cmd"
            $wc = New-Object System.Net.WebClient
            $wc.DownloadFile($url, $destPath)
            $Global:MAS_File = $destPath
...
function Invoke-MASInteractive {
    $masFile = Deploy-MAS
    if ($masFile) {
        Start-Process "cmd.exe" -ArgumentList "/c `"$masFile`"" -Verb RunAs
    }
}
...
function Invoke-MAS_HWID {
    $masFile = Deploy-MAS
    if ($masFile) {
        Start-Process "cmd.exe" -ArgumentList "/c `"$masFile`" /HWID" -Verb RunAs
    }
}
...
function Invoke-MAS_TSforge {
    $masFile = Deploy-MAS
    if ($masFile) {
        Start-Process "cmd.exe" -ArgumentList "/c `"$masFile`" /Z-Windows" -Verb RunAs
    }
}
...
function Invoke-MAS_Ohook {
    $masFile = Deploy-MAS
    if ($masFile) {
        Start-Process "cmd.exe" -ArgumentList "/c `"$masFile`" /Ohook" -Verb RunAs
    }
}
...
function Invoke-MAS_KMS {
    $masFile = Deploy-MAS
    if ($masFile) {
        Start-Process "cmd.exe" -ArgumentList "/c `"$masFile`" /K-WindowsOffice" -Verb RunAs
    }
}
...
$VersionLabel.Text = "v13.0 MAS OFFLINE"
...
$InstallBtn.Add_Click({
    ...
    if (-not $HasWinget -and -not $HasChoco) {
        $R = [System.Windows.Forms.MessageBox]::Show("No hay gestores. Instalar Chocolatey?", "SHADOWIEX", 4)
        if ($R -ne 6) { return }
    ...
    $ProgF.Close()
    if ($Global:Cancelled) { Update-Status "Cancelado" "warning" }
...
$TweaksList = @(
    @{Name="Essential Tweaks"; Desc="Optimizaciones esenciales"; Action={
        ...
        try {
            ...
            @("DiagTrack","dmwappushservice") | ForEach-Object { try { Set-Service $_ -StartupType Disabled -EA 0 } catch {} }
...
# Verificar estado de MAS
$masStatus = Find-MAS
$masStatusText = if ($masStatus) { "MAS detectado - Listo para activar" } else { "MAS no encontrado - Se descargara" }
...
$ActivateDesc.Text = $masStatusText
...
$ActivateDesc.ForeColor = $masStatusColor
$TabActivate.Controls.Add($ActivateDesc)

$ActList = @(
    @{Name="MAS INTERACTIVO"; Desc="Menu completo de activacion MAS"; Color="Success"; Action={Invoke-MASInteractive}},
    @{Name="ACTIVAR WINDOWS"; Desc="HWID para Windows 10/11"; Color="Primary"; Action={Invoke-MAS_HWID}},
    @{Name="ACTIVAR OFFICE"; Desc="Ohook para Office"; Color="Primary"; Action={Invoke-MAS_Ohook}},
...
$InfoLabel.Text = "Distribuir Shadowiex_v13.ps1 junto con MAS_AIO.cmd"
...
$InfoLabel2.Text = "HWID: Win10/11 | TSforge: Todos | Ohook: Office"
...
# MAS Status
$MASLabel = New-Object System.Windows.Forms.Label
$masFound = Find-MAS
$MASLabel.Text = "MAS_AIO.cmd: " + $(if($masFound){"[ENCONTRADO]"}else{"[NO ENCONTRADO]"})
$MASLabel.Location = New-Object System.Drawing.Point(20, 110)
$MASLabel.Size = New-Object System.Drawing.Size(350, 22)
$MASLabel.Font = $Global:Fonts.Normal
$MASLabel.ForeColor = if($masFound){$Global:Theme.Success}else{$Global:Theme.Warning}
$TabConfig.Controls.Add($MASLabel)

$SysTitle = New-Object System.Windows.Forms.Label
$SysTitle.Text = "INFORMACION DEL SISTEMA"
$SysTitle.Location = New-Object System.Drawing.Point(20, 150)
...
$CreditTitle.Text = "SHADOWIEX v13.0 MAS OFFLINE"
...
$MASStatusStrip = New-Object System.Windows.Forms.ToolStripStatusLabel
$MASStatusStrip.Text = "  MAS: " + $(if($masFound){"OK"}else{"--"})
$MASStatusStrip.ForeColor = if($masFound){$Global:Theme.Success}else{$Global:Theme.Warning}
$StatusStrip.Items.Add($MASStatusStrip)

$Form.Controls.Add($StatusStrip)

# Mostrar
$Global:AllChecked = $false
Update-Status "SHADOWIEX v13.0 - MAS OFFLINE - Listo"
